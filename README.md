...
try:
    ...
    ER_Triageenv = ErTriageEnv.from_docker_image("ER_Triage-env:latest")

    # Reset
    result = ER_Triageenv.reset()
    ...
    for msg in messages:
        result = ER_Triageenv.step(ErTriageAction(message=msg))
        print(f"Sent: '{msg}'")
...
### Action
**ErTriageAction**: Contains a single field
- `message` (str) - The message to echo back

### Observation
**ErTriageObservation**: Contains the echo response and metadata
...
If you already have a Er Triage environment server running, you can connect directly:

```python
from ER_Triage import ErTriageEnv

# Connect to existing server
ER_Triageenv = ErTriageEnv(base_url="<ENV_HTTP_URL_HERE>")

# Use as normal
result = ER_Triageenv.reset()
result = ER_Triageenv.step(ErTriageAction(message="Hello!"))
```

Note: When connecting to an existing server, `ER_Triageenv.close()` will NOT stop the server.
...
with ErTriageEnv(base_url="http://localhost:8000") as env:
    ...
    for msg in ["Hello", "World", "!"]:
        result = env.step(ErTriageAction(message=msg))
        print(f"Echoed: {result.observation.echoed_message}")
```
